...
const exportPostPDF = async (req, res) => {
    try {
        const users = await heroModel.getAllHeros();
        ...
        users.forEach((hero) => {
            doc.text(`${hero.id} | ${hero.name} | ${hero.publisher}`);
        });
...
module.exports = { exportPostPDF }; // Certifique-se de que está exportando corretamente