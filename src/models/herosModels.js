...
const getHeros = async (name) => {
    if (!name) {
        ...
        const result = await pool.query("SELECT * FROM heros WHERE name ILIKE $1", [`%${name}%`]);
        return result.rows;
    }
};
...
const updateHero = async (id, name, publisher_name) => {
    const result = await pool.query(
        "UPDATE heros SET name = $1, publisher_name = $2 WHERE id = $3 RETURNING *",
        [name, publisher_name, id]
    );
    return result.rows[0];
};
...
const deleteHero = async (id) => {
    const result = await pool.query(
        "DELETE FROM heros WHERE id = $1 RETURNING *",
        [id]
    );
    return result.rows[0];
};
